$tfpt_assm = "${Env:ProgramFiles(x86)}\Microsoft Team Foundation Server 2010 Power Tools\Microsoft.TeamFoundation.PowerTools.PowerShell.dll"
...
function Register-TfptCmdlets64 {
  if ($isWin7AndUp) {
    if (-not (Test-Path $tfpt_reg_key)) {
      regedit /s $PSScriptRoot\tfpt-x64.reg
    }
  }
}

# Make the TFPT PowerShell Cmdlets available
function Register-TfptCmdlets {
